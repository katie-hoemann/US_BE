...
translate <- 0 # SET TO 1 TO CALL DEEPL API; keep off to not overuse the service
my_key <- ## ADD KEY HERE
...
DF_Verbose <- separate(DF_Verbose, "Filename", c("PPID", "IID", "Text1", "Text2", "Sentence"), ";")
DF_Verbose$PPID <- gsub("-INTERVIEW_s1|-INTERVIEW_s2|-INTERVIEW_s3|-INTERVIEW_s4", "", DF_Verbose$PPID)
DF_Verbose <- subset(DF_Verbose, select = -c(IID, Text1, Text2))
...
setwd(paste0("C:/Users/Katie/Documents/MEH/",dataSet,"/For analysis_split by sentence_400 files_corrected_3 Mar 24_318056 320029 removed"))
DF_Text = data.frame(readr::read_csv(paste0("ERC Dutch_split by sentence_400 files_corrected_3 Mar 24_",situationType,"_318056 320029 removed.csv"))) #https://stackoverflow.com/questions/19610966/invalid-input-causes-read-csv-to-cut-off-data
setwd(paste0("C:/Users/Katie/Documents/MEH/",dataSet,"/MEH/",fileDate," analyses/",situationType,"/PCA results_",numComponents)) #jump to PCA results directory
...
if (translate == 1) {
  dirName <- paste0(Sys.Date(),"_Word clouds_EN")
  dir.create(dirName, showWarnings = FALSE)
  for (col in colnames(DF_Word_Clouds)[-1]) {
    words <- DF_Word_Clouds$Term[DF_Word_Clouds[[col]] > 0]
    words_EN <- toEnglish2(text = words, source_lang = "NL", auth_key = my_key)
    loadings <- DF_Word_Clouds[[col]][DF_Word_Clouds[[col]] > 0]*100
    output <- file.path(dirName, paste("wordcloud", col, ".png", sep = "_"))
    png(output)
    wordcloud(words=words_EN, freq=loadings, scale=c(3, 0.25), min.freq = 10, random.order=FALSE, rot.per=0, colors=brewer.pal(4, "Dark2"))
    title(main = col)
    dev.off()
  }
}
...
#SUMMARY FILE (https://stackoverflow.com/questions/73713965/translating-text-within-r)
...
if (translate == 1) {
  text1_EN <- toEnglish2(text = DF_Summary$Text1, source_lang = "NL", auth_key = my_key)
  text2_EN <- toEnglish2(text = DF_Summary$Text2, source_lang = "NL", auth_key = my_key)
  text3_EN <- toEnglish2(text = DF_Summary$Text3, source_lang = "NL", auth_key = my_key)
  top_words_string_EN <- toEnglish2(text = top_words_string, source_lang = "NL", auth_key = my_key)
  DF_Summary <- add_column(DF_Summary, Text1_EN = text1_EN, Text2_EN = text2_EN, Text3_EN = text3_EN, TopWords_EN = top_words_string_EN)
}
